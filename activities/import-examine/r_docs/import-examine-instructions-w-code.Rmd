...
-   in the "setup" code chunk, load this package with the `library()` function:
...
#### 1.3 Install a package using a console command  

There are two packages we will use today that are not part of the docker image that was built for this class (remember I said I left a few packages uninstalled so you could get a little practice managing packages).   
- The first package is called `kabelExtra` and it contains functions that are useful for formatting tables. To install the first package type this command in the console at the `>` prompt (then run the command):  
    `install.packages("kableExtra")`  
- The second package is called `Hmisc` and it contains functions that we'll use to calculate confidence intervals around a sample mean. To install the second package, `Hmisc`, type this command in the console at the `>` prompt (then run the command):  
    `install.packages("Hmisc")`  
- the command downloads and installs the package from the Comprehensive R Archive Network (CRAN) repository, a central place where R packages get registered and shared.  
...
-   Calculate the Shapiro-Wilk statistic for deviation from a normal distribution (use the function shapiro.test() on the Height data). shapiro.test() is an function that only accepts a vector of values (doesn't accept a "data" argument), so to use the usual piping syntax, you need the `pull()` function to "pull" the values from the Height column of the tibble and pass the vector to the `shapiro.test` function. The code is `nhanes_tib  |> pull(Height) |> shapiro.test()` - or alternatively you ditch the piping syntax and use `shapiro.test(nhanes_tib$Height)`.  Although the first example may look more complicated, the piping syntax can be very useful when there are additional piping steps before calling a function.
...
p1; p2; p3
nhanes_tib |> drop_na(Height) |> pull(Height) |> shapiro.test()
```
</div>
...
height_quants <- nhanes_tib |> drop_na(Height) |> filter(Age>=18) |>
  pull(Height) |> quantile(c(.25,.5,.75), na.rm = TRUE)
...
p1; p2; p3
nhanes_tib |> drop_na(Height) |> filter(Age>=18) |> pull(Height) |> shapiro.test()
```
...
p1; p2; p3
cort_tib |> drop_na(cortisol_t1) |> pull(cortisol_t1) |> shapiro.test()
```
</div>
...
cort_tib |> drop_na(logcortisol_t1) |> 
  pull(logcortisol_t1) |> shapiro.test()
```
</div>
