...
This is caused by a change between version 0.8.5 and 0.8.6 of the "effectsize" package. Similar errors may occur in different functions when you try to use the formula syntax `y ~ x` with the paired = TRUE option. This error is meant to prevent people from using the formula syntax when their data are not sorted (notice that the formula does not include the `pnum_rm` value that defines how observations are paired).  
One solution is to use `x = ` and `y = ` and select audio and text condition values separately instead of formula syntax. Check the solution code to see exactly how.   
...
effectsize::cohens_d(Hire_Rating ~ condition_factor, data = ratings_rm_tib, 
  ...
  knitr::kable(caption = "Effect size Cohen's d (pooled)", digits = 3) |> 
  kableExtra::kable_styling(full_width = FALSE)
#2 -it doesn't hurt to repeat the arrange() function here
ratings_rm_tib <- ratings_rm_tib |> dplyr::arrange(pnum_rm) 
effectsize::cohens_d(
  x = ratings_rm_tib$Hire_Rating[ratings_rm_tib$condition_factor == "audio"],
  y = ratings_rm_tib$Hire_Rating[ratings_rm_tib$condition_factor == "text"],
  pooled_sd = FALSE, paired = TRUE, alternative = "two.sided") |>     
  knitr::kable(caption = "Effect size Cohen's dz (dependent)", digits = 3) |> 
  kableExtra::kable_styling(full_width = FALSE)

### another approach - use the repeated_measures_d() function, but this gives a slightly different answer even though the documentation says it uses the same calculation
# effectsize::repeated_measures_d(Hire_Rating ~ condition_factor | pnum_rm,
#                                 data = ratings_rm_tib,
#                                 pooled_sd = FALSE,
#                                 paired = TRUE,
#                                 alternative = "two.sided",
#                                 method = "z") |>
#   knitr::kable(caption = "Effect size Cohen's dz using repeated_measures_d()", digits = 3) |>
#   kableExtra::kable_styling(full_width = FALSE)
...
- How would you depict the findings in the between-subjects design example? In the within-subjects design example?  
    - check out [Figure 7](https://journals-sagepub-com.proxy.libraries.rutgers.edu/cms/10.1177/0956797615572906/asset/600417a7-39f4-4a6f-961c-f1a1917b30fc/assets/images/large/10.1177_0956797615572906-fig7.jpg) to see how [Schroeder & Epley (2015)](https://doi.org/10.1177/0956797615572906) depicted the between-subjects data in their paper.  
