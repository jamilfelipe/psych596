...
To do the "extra" part of the activities you will lso need the packages "ggpubr" and "sjPlot"    
...
# The confusing {.$blahblah} syntax is required because gmodels is not a tidyverse function, and is not designed to accept the pipe input in a way that allows you to refer to specific columns. We encountered this issue in the import-examine activity (and used the "pull" function to get around it, but that only works for 1 column). Here is a difference strategy where we use a different kind of pipe ("exposed pipe": %$% - it "exposes" the column names to the function), which is part of the "magrittr" package  
library("magrittr")
conting_table <- lumos_tib %>%   
  filter(edu_cat != "unspecified" & english_nativelang != "unspecified") %$% 
  gmodels::CrossTable(edu_cat, english_nativelang, expected=TRUE,
...
print(semipart)
# partial (this time using "exposed pipe" %$% syntax)
